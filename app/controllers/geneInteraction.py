...
def get_gene_network(dataset_ID: int = None, disease_name=None,
    ...
    # or filter them by a centrality metric. When neither is requested we keep EVERY gene that
    # appears in the (already filtered) edges, so genes lacking a network_analysis row are not
    ...
    if use_network_analysis:
        ...
        na_rows = db.session.execute(
            db.select(models.networkAnalysis).filter(
                models.networkAnalysis.sponge_run_ID.in_(run_query),
                models.networkAnalysis.gene_ID.in_(candidate_ids)
            )
        ).scalars().all()
        na_by_gene = {row.gene_ID: row for row in na_rows}
        ...
        gene_objs = {}
        missing_ids = [gid for gid in candidate_ids if gid not in na_by_gene]
        if missing_ids:
            objs = db.session.execute(
                db.select(models.Gene).filter(models.Gene.gene_ID.in_(missing_ids))
            ).scalars().all()
            gene_objs = {g.gene_ID: g for g in objs}

        all_nodes = []
        for gid in candidate_ids:
            if gid in na_by_gene:
                all_nodes.append(na_by_gene[gid])
            elif gid in gene_objs:
                synthetic = models.networkAnalysis()
                synthetic.gene_ID = gid
                synthetic.gene = gene_objs[gid]
        ...
        if nodeSorting:
            node_ranks = {node.gene_ID: [] for node in filtered_nodes}
            for col in nodeSorting:
                ...
                for idx, node in enumerate(sorted_for_col):
                    ...
                    if val != prev_val:
                        current_rank = idx + 1
                        prev_val = val
                    node_ranks[node.gene_ID].append(current_rank)

            def get_avg_rank(n):
                ranks = node_ranks[n.gene_ID]
        ...
        # the genes that have none, so the response shape (networkAnalysisSchema) is preserved.
        na_rows = db.session.execute(
            db.select(models.networkAnalysis).filter(
                models.networkAnalysis.sponge_run_ID.in_(run_query),
                models.networkAnalysis.gene_ID.in_(candidate_ids)
        ...
        if missing_ids:
            ...
            gene_objs = db.session.execute(
                db.select(models.Gene).filter(models.Gene.gene_ID.in_(missing_ids))
            ).scalars().all()
            for gene_obj in gene_objs:
                synthetic = models.networkAnalysis()
                synthetic.gene_ID = gene_obj.gene_ID
                synthetic.gene = gene_obj
                synthetic.sponge_run = representative_run
                synthetic.betweenness = None
                synthetic.eigenvector = None
                synthetic.node_degree = None
    ...
    node_gene_ids = set([node.gene_ID for node in node_results])
    edge_query = edge_query.filter(
        and_(
